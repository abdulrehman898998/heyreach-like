import FacebookBot from './facebookBot.js';
...
/**
 * The main function to run the Facebook automation.
 */
async function runFacebookAutomation(config, logCallback) {
    logCallback('--- Starting Facebook DM Automation ---');
    let bot = null;

    // Validate core configuration
    if (!config?.facebook?.accounts?.length) {
        throw new Error('No Facebook accounts configured');
    }
    ...
    try {
        ...
        const accounts = [...config.facebook.accounts];
        shuffleArray(accounts);
        if (!accounts || accounts.length === 0) {
          throw new Error('Valid Facebook accounts array not received');
        }
        
        logCallback(`👥 Using ${accounts.length} Facebook accounts`);
        ...
        for (let i = 0; i < profilesToProcess.length; i++) {
            ...
            try {
                ...
                if (accountUsageCounter % config.facebook.messagesPerAccount === 0) {
                    if (bot) {
                    ...
                    bot = new FacebookBot(currentAccount, config.proxy, userDataDir);
                ...
                if (result === true) {
                    ...
                    // Write success status to Google Sheet column C
        ...
        logCallback(`❌ An unexpected error occurred during Facebook automation: ${err.message}`);
        console.error('Facebook automation error:', err);
    } finally {
        if (bot) {
    ...
    logCallback('--- Facebook automation script finished. ---');
}

export { runFacebookAutomation }; 